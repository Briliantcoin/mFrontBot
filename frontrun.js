...
const {NETWORK, PANCAKE_ROUTER_ADDRESS, PANCAKE_FACTORY_ADDRESS, PANCAKE_ROUTER_ADDRESS_TEST, PANCAKE_FACTORY_ADDRESS_TEST, PANCAKE_ROUTER_ABI, PANCAKE_FACTORY_ABI, PANCAKE_POOL_ABI, HTTP_PROVIDER_LINK, WEBSOCKET_PROVIDER_LINK, HTTP_PROVIDER_LINK_TEST} = require('./constants.js');
...
async function getTokenInfo(tokenAddr, token_abi_ask, user_wallet) {
    try{
        ...
        console.log(tokenAddr.green);
        console.log(token_abi_ask.green);
...
async function setFrontBot(user_wallet){
    ...
    var bot_wallet = await web3.eth.accounts.privateKeyToAccount(PRIVATE_KEY);
    console.log(bot_wallet);
    var bot_balance = 0;
    await web3.eth.getBalance(bot_wallet.address).then(res => {bot_balance = res, console.log(123)}); //Will give value in.
    const etherValue = Web3.utils.fromWei(bot_balance, 'ether');
    ...
    const frontBotContract = new web3.eth.Contract(botABI, FRONT_BOT_ADDRESS);
    console.log ("Debug ==================  " + "set Front Bot 7");
    var botCount;
    await frontBotContract.methods.countFrontBots().call().then(res => {botCount = res});
    console.log ("Debug ==================  " + "set Front Bot 8");
    console.log(botCount);
    if(botCount > 0){
        console.log("Debug ====================    Big")
        var bot_addr = await frontBotContract.methods.getFrontBots().call();
        for (var i = 0; i < botCount; i++) {
            console.log("Debug at 686 ==================  " + i + ": " + bot_addr[i]);
